...
#' @param as.is should strings be left as strings or converted to factors?
#' @export
melt <- function(data, ..., na.rm = FALSE, value.name = "value", as.is = TRUE) {
  UseMethod("melt", data)
}
...
melt.data.frame <- function(data, id.vars, measure.vars, variable.name = "variable", ..., na.rm = FALSE, value.name = "value", as.is = TRUE) {
  ...
  variable <- rep(var$measure, each = nrow(data))
  if(!as.is){
    variable <- factor(variable, levels = var$measure)
  }
...
#' melt(a)
melt.array <- function(data, varnames = names(dimnames(data)), ..., na.rm = FALSE, value.name = "value", as.is = TRUE) {
  var.convert <- function(x){
    if(is.character(x)){
      type.convert(x, as.is = as.is)
    }else {
      x
    }
  }
